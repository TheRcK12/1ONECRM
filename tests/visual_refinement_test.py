...
assert "APP_VERSION = \"2.6.8-beta.1\"" in (ROOT / "one_crm_server.py").read_text(encoding="utf-8")
assert 'org.opencontainers.image.version="2.6.8-beta.1"' in DOCKER
assert "one_crm_productivity.py" in DOCKER

# A versão visual deve continuar servindo assets com cache busting.
assert "app.css?v=2.6.8" in INDEX
assert "app.js?v=2.6.8" in INDEX
